...
def InitHost():
	...
	hook_HostFromPrefsByProtocol.currentHost = MockHost()
	return hook_HostFromPrefsByProtocol.currentHost

def RunClient(args, hostBlockChain='litecoin'):
	convertedArgs = []
	for arg in args:
	...
	assert path.isdir(dataDirectory)
	host = hook_HostFromPrefsByProtocol.currentHost
...
def GetStateInfo(includePending=False, forceRescan=False):
	args = []
	if forceRescan:
...
class Test(unittest.TestCase):
	...
	def test_burn_pay_and_sync_output(self):
		...
		output, result = RunClient(['get_receive_address'])
	...
	def test_refund_account_referenced_during_trade(self):
		...
		host._addUnspent(5*e(8))
		RunClient(['burn', '--amount', 1*e(8)])
		RunClient(['sell_offer', '--hostCoinOffered', 3*e(7)//2, '--exchangeRate', '0.5', '--blocksUntilExpiry', '20'])
		deposit = 3*e(7) // 16
		output, result = RunClient(['get_balance'])
		...
		host._setOwner('2')
		host._addUnspent(500000000)
		RunClient(['burn', '--amount', 2*e(8)])
		RunClient(['buy_offer', '--swapBillOffered', 29900000, '--exchangeRate', '0.5'])
		...
		RunClient(['pay', '--amount', Amounts.ToString(1*e(8)-deposit-Constraints.minimumSwapBillBalance), '--toAddress', payTargetAddress])
		output, result = RunClient(['get_balance'])
		self.assertDictEqual(result, {'balance': '0'})
		host._setOwner('recipient')
		output, result = RunClient(['get_balance'])
	...
	def test_receiving_account_referenced_during_trade(self):
		...
		host._setOwner('1')
		host._addUnspent(500000000)
		RunClient(['burn', '--amount', 100000000])
		RunClient(['sell_offer', '--hostCoinOffered', 29900000//2, '--exchangeRate', '0.5'])
		output, result = RunClient(['get_balance'])
		...
		host._setOwner('2')
		host._addUnspent(500000000)
		RunClient(['burn', '--amount', 200000000])
		RunClient(['buy_offer', '--swapBillOffered', 30000000, '--exchangeRate', '0.5'])
		output, result = RunClient(['get_balance'])
		# the offers can't match, because this would result in a remainder below minimum exchange
		...
		payTargetAddress = result['receive_address']
		host._setOwner('2')
		RunClient(['pay', '--amount', '1.7', '--toAddress', payTargetAddress])
		output, result = RunClient(['get_balance'])
		self.assertDictEqual(result, {'balance': '0'})
		host._setOwner('recipient')
		output, result = RunClient(['get_balance'])
	...
	def test_ltc_trading(self):
		...
		RunClient(['burn', '--amount', 6*e(7)])
		host._setOwner('dave')
		RunClient(['burn', '--amount', 7*e(7)])
		# alice and bob both want to buy LTC
		# clive and dave both want to sell
		# alice makes buy offer
		host._setOwner('alice')
		RunClient(['buy_offer', '--swapBillOffered', 3*e(7), '--exchangeRate', '0.5', '--blocksUntilExpiry', '100'])
		info = GetStateInfo()
		...
		RunClient(['buy_offer', '--swapBillOffered', 1*e(7), '--exchangeRate', '0.25'])
		info = GetStateInfo()
		ownerBalances = GetOwnerBalances(host, ownerList, info['balances'])
		...
		host._setOwner('clive')
		host._addUnspent(100000000)
		RunClient(['sell_offer', '--hostCoinOffered', 10000000//4, '--exchangeRate', '0.25'])
		...
		output, result = RunClient(['get_pending_exchanges'])
		expectedResult = [
		...
		RunClient(['sell_offer', '--hostCoinOffered', 2*e(7) * exchangeRate // Amounts.percentDivisor, '--exchangeRate', Amounts.PercentToString(exchangeRate), '--blocksUntilExpiry', '100'])
		info = GetStateInfo()
		ownerBalances = GetOwnerBalances(host, ownerList, info['balances'])
		...
		host._setOwner('dave')
		RunClient(['complete_sell', '--pendingExchangeID', '1'])
		info = GetStateInfo()
		...
		host._setOwner('alice')
		info = GetStateInfo(includePending=False, forceRescan=True)
	...
	def test_backed_ltc_sell(self):
		...
		host._addUnspent(2*e(12))
		RunClient(['burn', '--amount', 1*e(12)])
		RunClient(['back_sells', '--backingSwapBill', 1*e(12), '--transactionsBacked', '1000', '--blocksUntilExpiry', '20', '--commission', '0.0625'])
		output, info = RunClient(['get_balance'])
		self.assertDictEqual(info, {'balance': '0'})
		self.assertEqual(host._nextBlock, 3)
		output, result = RunClient(['get_sell_backers'])
		...
		RunClient(['burn', '--amount', 3*e(8)])
		RunClient(['buy_offer', '--swapBillOffered', 3*e(8), '--exchangeRate', '0.5', '--blocksUntilExpiry', '100'])
		info = GetStateInfo()
		self.assertEqual(info['numberOfHostCoinBuyOffers'], 1)
		self.assertEqual(info['numberOfHostCoinSellOffers'], 0)
		self.assertEqual(info['numberOfPendingExchanges'], 0)
		...
		RunClient(['sell_offer', '--hostCoinOffered', ltcOffered+commission, '--exchangeRate', '0.5', '--backerID', 0, '--includesCommission'])
		info = GetStateInfo()
		ownerBalances = GetOwnerBalances(host, ownerList, info['balances'])
	...
	def test_backed_ltc_sell_add_commission(self):
		...
		ltcOffered = 15*e(7)
		commission = ltcOffered // 16
		RunClient(['sell_offer', '--hostCoinOffered', ltcOffered, '--exchangeRate', '0.5', '--backerID', 0])
		output, result = RunClient(['get_sell_offers'])
	...
	def test_backed_sell_matches_multiple(self):
		...
		host._addUnspent(5*e(12))
		host._setOwner('buyer')
		RunClient(['burn', '--amount', 6*e(9)])
		RunClient(['buy_offer', '--swapBillOffered', 2*e(9), '--exchangeRate', '0.5', '--blocksUntilExpiry', '100'])
		RunClient(['buy_offer', '--swapBillOffered', 4*e(9), '--exchangeRate', '0.5', '--blocksUntilExpiry', '100'])
		info = GetStateInfo()
		self.assertEqual(info['numberOfHostCoinBuyOffers'], 2)
		self.assertEqual(info['numberOfHostCoinSellOffers'], 0)
		self.assertEqual(info['numberOfPendingExchanges'], 0)
		...
		host._setOwner('backer')
		RunClient(['burn', '--amount', 1*e(12)])
		RunClient(['back_sells', '--backingSwapBill', 1*e(12), '--transactionsBacked', '10', '--blocksUntilExpiry', '100', '--commission', '0.1'])
		host._setOwner('seller')
		ltcOffered = 6*e(9)//2
		deposit = 6*e(9)//Constraints.depositDivisor
		ltcCommission = ltcOffered // 10
		RunClient(['sell_offer', '--hostCoinOffered', ltcOffered + ltcCommission, '--exchangeRate', '0.5', '--backerID', 0, '--includesCommission'])
		info = GetStateInfo()
		self.assertEqual(info['numberOfHostCoinBuyOffers'], 0)
		self.assertEqual(info['numberOfHostCoinSellOffers'], 0)
		self.assertEqual(info['numberOfPendingExchanges'], 2)
	...
	def test_pay_on_proof_of_receipt(self):
		...
		host._setOwner('someoneElse')
		self.assertRaisesRegexp(ExceptionReportedToUser, "The secret for this pending payment is not known", RunClient, ['reveal_secret_for_pending_payment', '--pendingPaymentID', 0])
		host._setOwner(host.defaultOwner)		
