...
class Test(unittest.TestCase):
	...
	def test(self):
		...
		self.assertEqual(tx.typeCode, 5)
		...
		self.assertEqual(tx.source, bob)
		self.assertSequenceEqual(tx.destinations, [bob2])
		...
		self.assertIsInstance(decodedTX, TransactionTypes.ForwardToFutureVersion)
		...
		self.assertEqual(tx.source, bob)
		assert not hasattr(tx, 'controlAddressAmount')
		assert not hasattr(tx, 'destinations')
		assert not hasattr(tx, 'destinationAmounts')
		amount, maxBlock, extraData = tx.encode()
		self.assertEqual(amount, 111)
		self.assertEqual(maxBlock, 0xffffffff)
		...
		self.assertEqual(tx.typeCode, 4)
