...
class BuyOffer(object):
	def __init__(self, swapBillOffered, rate):
		...
		self.rate = rate
	def subtractExchanged(exchangeSwapBill):
		assert exchangeSwapBill < self.swapBillOffered
		remainder = self._swapBillOffered - exchangeSwapBill
		if _ltcWithExchangeRate(self.rate, self._swapBillOffered) < _minimumExchangeLTC:
...
class SellOffer(object):
	def __init__(self, swapBillDeposit, ltcOffered, rate):
		...
		self.rate = rate
	def subtractExchanged(exchangeLTC, exchangeSwapBillDeposit):
		...
		if remainder < _minimumExchangeLTC:
			# ** offer must not be modified at this point
			raise RemainderIsBelowMinimumExchange()
		self.ltcOffered = remainder
		self.swapBillDeposit -= exchangeSwapBillDeposit
...
def MatchOffers(buy, sell):
	...
	if ltcToBeExchanged <= sell._ltcOffered:
		...
		if ltcToBeExchanged < sell._ltcOffered:
			sell.subtractExchanged(ltcToBeExchanged, exchange.swapBillDeposit)
		...
		if swapBillToBeExchanged < buyDetails.swapBillOffered: # TODO check whether this comparison is actually required
			buy.subtractExchanged(swapBillToBeExchanged)
			outstandingBuy = buy
